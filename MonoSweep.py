...
def main():
    ...
    while True:
        ...
        if pilihan == '1':
            ...
            print()
            # Tanyakan konfigurasi keamanan PRO sebelum menjalankan modul duplicate cleaner
            while True:
                print(f"\n{BOLD}{YELLOW}Konfigurasi Keamanan Pro:{RESET}")
                print(f"  1. Verifikasi Konten Aktif (Nama & MD5 - lebih aman, lebih lambat)")
                print(f"  2. Verifikasi Cepat Aktif (Nama saja - lebih cepat)")
                pilihan_mode = input(f"\n{BOLD}[?] Pilih mode (1/2) [default 2]: {RESET}").strip()
                if pilihan_mode == '' or pilihan_mode == '2':
                    VERIFIKASI_ISI_FILE = False
                    break
                elif pilihan_mode == '1':
                    VERIFIKASI_ISI_FILE = True
                    break
                else:
